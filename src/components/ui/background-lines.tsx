...
export const BackgroundLines = ({
  ...
  svgOptions?: {
    duration?: number;
    pathCount?: number; // Control number of paths to render (for performance)
  };
}) => {
  ...
  return (
    <div
      className={cn(
        "relative min-h-[20rem] w-full bg-white dark:bg-black",
        className
      )}
    >
...
// Modify path variants for better performance on mobile
const getPathVariants = (isMobile: boolean) => ({
  initial: { 
    strokeDashoffset: 800, 
    strokeDasharray: isMobile ? "80 800" : "50 800" 
  },
  animate: {
    strokeDashoffset: 0,
    strokeDasharray: isMobile ? "40 800" : "20 800",
    opacity: [0, 1, 1, 0],
  },
});
...
const SVG = ({
  ...
  const paths = [
    ...
    // More paths removed for brevity but included in the full component
  ];
  ...
  const colors = [
    ...
    "#4D3568", "#9F39A5", "#363636", "#860909", "#6A286F", "#604483"
  ];

  // Determine how many paths to render based on device performance
  const pathCount = svgOptions?.pathCount || (isMobile ? 10 : 20);
  const pathsToRender = paths.slice(0, pathCount);
  const pathVariants = getPathVariants(isMobile);

  // Calculate reasonable animation duration based on device
  const baseDuration = svgOptions?.duration || (isMobile ? 15 : 10);
  ...
  return (
    <motion.svg
      ...
      preserveAspectRatio="xMidYMid slice" // This ensures the SVG scales correctly on all devices
    >
      {/* First set of paths */}
      {pathsToRender.map((path, idx) => (
        <motion.path
          ...
          transition={{
            ...
            repeat: Infinity,
            repeatType: "loop",
            delay: Math.floor(Math.random() * (isMobile ? 5 : 10)),
            repeatDelay: Math.floor(Math.random() * (isMobile ? 5 : 10) + 2),
          }}
          key={`path-first-${idx}`}
        />
      ))}

      {/* Duplicate paths with different animation timing for more visual interest */}
      {!isMobile && pathsToRender.slice(0, pathCount/2).map((path, idx) => (
        <motion.path
          d={path}
          stroke={colors[(idx + pathCount/2) % colors.length]}
          strokeWidth="2.3"
          strokeLinecap="round"
          variants={pathVariants}
          initial="initial"
